# Fixed main issues
when using numpy above version 1.25.0, meet the error: If using `np.array(obj, copy=False)` replace it with `np.asarray(obj)` to allow a copy when needed (no behavior change in NumPy 1.x). This version fixed the bugs; this branch from v0.9.2. 
The guide for upgrade numpy link:
